...
Alignment with the platform role model (see FEAT-004/FEAT-023 in `./FeatureCatalog.md` and FR-120–FR-122 in `./PRD.md`): the built-in base roles are `PLATFORM_ADMIN`, `TENANT_ADMIN`, `ANALYST`, and `VIEWER`; the remaining six (`ENGINEERING_MANAGER`, `TEAM_LEAD`, `MEMBER`, `RELEASE_MANAGER`, `EXECUTIVE_VIEWER`, `SECURITY_AUDITOR`) ship as **seeded persona role templates** composed from fine-grained permissions on top of the built-in `ANALYST`/`VIEWER` bases, provisioned by default at tenant creation (UC-001 in `./UseCases.md`) and editable per tenant. Tenant admins may clone and adjust templates (permission preview supported) but cannot grant permissions they do not themselves administer. Persona-to-role mapping below always names the template; deployments may rename templates without affecting this document's traceability, because use cases in `./UseCases.md` reference personas, not role names.
...
- **Goals:** Install and upgrade EIP with zero data loss; connect Jira, GitHub, Confluence, SonarQube, and CI/CD systems (connector availability follows the roadmap phases in `./Roadmap.md` — Jira and GitHub first); keep sync pipelines healthy; configure local LLMs (Ollama/vLLM) so no data leaves the network; onboard tenants and delegate administration.
