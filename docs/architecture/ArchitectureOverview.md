...
|---|--------|-----------|---------------------------|
| D1 | **On-premise first, no SaaS dependencies** | Deployment model | Every runtime dependency (PostgreSQL 16, Redis 7, Kafka KRaft, MinIO, Keycloak, OTel Collector/Prometheus/Grafana) must be self-hostable via Docker Compose and Kubernetes/OpenShift. No cloud-managed services assumed. |
| D2 | **Air-gapped operation with local LLMs** | Security/compliance | LLM access goes through a provider SPI (Ollama, vLLM OpenAI-compatible, OpenAI-compatible generic, Anthropic-compatible, custom enterprise endpoint). All AI features must function with Ollama/vLLM on the local network; external providers are optional and configurable per tenant. No telemetry leaves the perimeter. |
| D3 | **Scale targets** | Capacity planning | Target envelope: up to 50 tenants per installation, 5,000 active engineers, 500 connected tool instances, 2,000 sync jobs/hour, sustained 1,000 domain events/second peak, 100M canonical work-item/SCM/CI records, 10M RAG chunks. Ingestion and analytics must scale horizontally without scaling the API tier. |
| D4 | **Multi-tenancy with hard isolation** | Security | Row-level tenant isolation: every tenant-scoped table carries `tenant_id` and is protected by Postgres RLS. Kafka event keys include `tenantId`; caches, vector search, RAG retrieval, and object-storage prefixes are tenant-partitioned. Cross-tenant reads are structurally impossible, not merely filtered. |
| D5 | **Partial-failure tolerance** | Availability | A failing connector, a saturated LLM endpoint, or a Kafka consumer lag must never take down dashboards or the API. Async boundaries (Kafka), circuit breakers, DLQs, and read models decouple failure domains. Degradation is graceful and explicit (see §9). |
| D6 | **Operability by small platform teams** | On-prem reality | Enterprise customers run EIP with 1–2 platform engineers. Deployment unit count must stay small (two backend deployables + infra), upgrades must be single-version Flyway migrations, and self-observability dashboards ship in `/infra/grafana`. |
| D7 | **Auditability and non-toxic analytics** | Compliance / product ethics | Every mutating action, secret access, and LLM call is audited. Metrics are team-level with stated caveats, uncertainty, and gaming risks; the platform is explicitly not an individual-surveillance or stack-ranking tool. |
| D8 | **Evolvability toward services** | Long-term roadmap | Module boundaries follow Spring Modulith conventions so that high-churn or high-scale modules (`eip-ingestion`, `eip-ai`) can be extracted to standalone services later without rewriting contracts. |
...
| `eip-analytics` | Metric engines (flow, DORA, quality, ops, team health), risk scoring, materialized read models | `eip-core`, `eip-tenancy` |
| `eip-ai` | Agent orchestration (LangChain4j), LLM provider SPI, RAG pipeline, MCP client/server | `eip-core`, `eip-tenancy`, `eip-analytics` (read-model queries via interface) |
| `eip-reports` | Report/template engine, exports, artifact library, scheduling | `eip-core`, `eip-tenancy`, `eip-ai`, `eip-analytics` |
...
**Dependency rules (enforced by Spring Modulith verification tests):**

1. `eip-core` depends on nothing; everything may depend on `eip-core`.
2. Only the two composition roots (`eip-app`, `eip-workers`) may depend on more than three modules.
3. No module accesses another module's tables or repositories; cross-module interaction is via exported Java interfaces (the module's `api` package) or Kafka events.
4. `eip-analytics` never calls `eip-connectors` or `eip-ingestion` — it consumes `eip.domain.*` topics only.
...
### 6.1 Tenancy
- Every tenant-scoped table has `tenant_id`; Postgres RLS policies bind to a per-connection `app.tenant_id` setting established by a tenancy filter (API) or the event envelope's `tenantId` (workers).
- Kafka messages are keyed `tenantId+entityId`, giving per-tenant/entity ordering and enabling tenant-aware partition assignment.
- Redis keys, MinIO object prefixes, vector-store collections/filters, and LLM budgets are all tenant-prefixed. RAG retrieval is permission-aware and tenant-isolated.
...
- **DLQs:** one DLQ per consumer group, named `<topic>.<group>.dlq`, with an operator replay tool.
...
| ADR-011 | CQRS-lite: materialized read models for dashboards, canonical model as write side | Accepted | Dashboard latency independent of ingestion load; read models rebuildable by topic replay |
...
- **Horizontal workers (primary lever).** `eip-workers` is stateless; all state lives in PostgreSQL/Redis/Kafka/MinIO. Scale-out is `replicas: N`. Kafka consumer groups rebalance partitions automatically. Distinct consumer groups per pipeline (ingestion, normalization, analytics, ai, reports) let operators scale pipelines independently — or run dedicated worker pools per group via Spring profile flags.
- **Partitioned topics.** `eip.raw.<connector>` and `eip.domain.*` topics are partitioned (default 12; sized to peak events/s ÷ per-consumer throughput). Keying by `tenantId+entityId` preserves per-entity ordering while spreading tenants across partitions. Hot tenants are contained to their key range and can be throttled per-tenant.
- **Read models.** Dashboards never query the canonical event stream; they hit pre-aggregated `rm_*` tables (per team/sprint/day grain) maintained by the metric engine. API read load therefore scales with users, not with ingestion volume. Read models are rebuildable by replaying `eip.domain.*` topics.
...
| Kafka broker unavailable | All async pipelines | Producer/consumer errors, lag alerts | `eip-app` outbox buffers domain events in PostgreSQL; API reads unaffected; ingestion/analytics/AI pause | Outbox relay drains on reconnect; consumers resume from committed offsets; no data loss |
...
| MinIO down | Artifacts + raw blobs | S3 client errors | Report generation fails fast to `eip.reports.jobs.<group>.dlq`; raw-blob ingestion retries; canonical JSONB path unaffected | Replay DLQ after recovery |
...
| Poison message / normalization bug | Single consumer group | Retry exhaustion metric | Message parked in `<topic>.<group>.dlq` with error context; pipeline continues past it | Fix, then operator replay from DLQ; idempotent consumers make replay safe |
