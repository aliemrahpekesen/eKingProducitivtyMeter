...
- A **Helm chart is a secondary option** for organizations whose platform tooling standardizes on Helm; Kustomize remains the source of truth and the chart must never diverge from it. A minimal, eip-app-only v0.1 chart shipped early (M5 Wave S2, DEBT-002/DEBT-005 packaging hardening) at `/infra/helm/eip` — see `./HelmDeployment.md` — as a smaller artifact for pilot/evaluation clusters; it does not implement the HA topology, autoscaling, or operator-managed data services this document specifies, and is superseded by (or reconciled against) the full Kustomize-generated chart at GA.
