...
- `GET /api/v1/connectors` — the tenant's connector registry (id, type, name, status, simulation), cursor-paginated in the `PageView` envelope (`items`/`nextCursor`/`hasMore`; params `cursor`, `limit` default 50/max 200, `sort` whitelisted to `name`/`-name`/`createdAt`/`-createdAt`, default `name`) per APIDesign §1.4. The cursor is signed (HMAC-SHA256) and expires (1h TTL, `ConnectorCursor`/`ApiCursorSigningProperties`, DEBT-010).
...
Errors are RFC 7807 problem+json with `/problems/*` `type` URIs (BackendPlan §10), documented per-operation in the OpenAPI contract via a shared `ProblemDetail` schema component and a global `OperationCustomizer` (`OpenApiConfig`) inferring 400/401/403/404 from each handler's annotations, plus endpoint-specific `@ApiResponse`s for 409/502/429 (DEBT-011). The generated OpenAPI 3 contract is served at `/v3/api-docs` (springdoc) and snapshotted to [`openapi/eip-openapi-v1.json`](openapi/eip-openapi-v1.json) (regenerate with `EIP_OPENAPI_EXPORT=1`; CI's `backend` job fails on committed↔generated drift — DEBT-011). Observability on these endpoints (metrics/traces/logs + problem `traceId`) lands with TASK-0012 ([DEBT-009](../../work/debt-register.md)). No owned tables/topics. This charter is updated in the same PR that adds them (RepositoryStructure.md §6 invariant 4).
